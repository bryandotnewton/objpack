...
function BufferPack() {
  ...
  m.unpack = function (fmt, a, p) {
    ...
    var fmtObj = tokenize(fmt),
      self = this;

    bBE = fmtObj.endian === 'big';

    function readFields(fields, o) {
      fields.forEach(function(field) {
        if(field.fields) {
          o[field.name] = {};
          return readFields(field.fields, o[field.name]);
        }
        ...
        if(field.type === 'S') {
          n = 0; //deal with empty null term strings
          while(a[p + n] !== 0) {
            n++;
          }
          n++; //one more for null byte
        }

        //get size of field type
        s = self._lenLut[field.type];
        ...
        switch (field.type) {
          case 'A': case 's': case 'S':
            o[field.name] = self._elLut[field.type].de(a, p, n);
            break;
          case 'c': case 'b': case 'B': case 'h': case 'H':
          case 'i': case 'I': case 'l': case 'L': case 'f': case 'd':
            el = self._elLut[field.type];
            o[field.name] = self._UnpackSeries(n, s, a, p)[0];
            break;
        }
    ...
    return readFields(fmtObj.fields, {});
